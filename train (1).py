    ...
    if (iteration % args.validation) == 0 or iteration == shared_cfg["iteration"]:
        with torch.no_grad():
            ...
            for j, data in enumerate(val_loader):
                ...
                # ramp up exp(-5(1 - t)^2)
                #coef1 = alg_cfg["consis_coef"] * math.exp(-5 * (1 - min(iteration/shared_cfg["warmup"], 1))**2)
                #ssl_loss1 = ssl_obj(input, output.detach(), model, unlabeled_mask) * coef
                loss1 += F.cross_entropy(output, target, reduction="none", ignore_index=-1).mean()
            ...
            loss1 = loss1 / float(len(val_dataset))
            metrics = {'accuracy': acc, 'loss':loss1}
            wandb.log(metrics)
